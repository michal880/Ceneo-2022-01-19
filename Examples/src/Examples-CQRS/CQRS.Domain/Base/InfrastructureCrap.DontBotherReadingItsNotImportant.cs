...
namespace CQRS.Domain.Base
{
  ...
  class PrivateReflectionDynamicObject : DynamicObject
  {
    ...
    private static IDictionary<string, IProperty> GetTypeProperties(Type type)
    {
      ...
      foreach (PropertyInfo prop in type.GetRuntimeProperties().Where(p => p.DeclaringType == type))
      {
        typeProperties[prop.Name] = new Property() { PropertyInfo = prop };
      }

      // Now, add all the fields
      foreach (FieldInfo field in type.GetRuntimeFields().Where(p => p.DeclaringType == type))
      {
    ...
    private static object InvokeMemberOnType(Type type, object target, string name, object[] args)
    {
      ...
      var method = type.GetTypeInfo().GetDeclaredMethods(name)
        .FirstOrDefault(f => f.GetParameters().Any(d => d.ParameterType == args[0].GetType()));
      if (method != null)
      {
